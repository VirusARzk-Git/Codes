...
void dfs(int node, int par, vector<vector<int>> &adj, vector<int> &dist, int &maxDist, int &farthest)
{
    for (int child : adj[node])
    {
        if (child != par)
        {
            ...
            dfs(child, node, adj, dist, maxDist, farthest);
        }
    }
}
...
void code()
{
    ...
    vector<int> dist(n, 0);
    dfs(0, -1, adj, dist, maxDist, farthest);
    fill(dist.begin(), dist.end(), 0);
    dfs(farthest, -1, adj, dist, maxDist, farthest);
    cout << maxDist << '\n';
}

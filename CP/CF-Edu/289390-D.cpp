...
void code()
{
    ...
    for (int i = 0; i < k; i++)
    {
        ...
        if (a == "cut")
            ...
            qtype = 0;
        queries.push_back({qtype, min(u, v), max(u, v)});
        if (qtype == 1)
            rem.insert({min(u, v), max(u, v)});
    }
    DSU dsu(n + 1);
    for (auto &i : edges)
        if (rem.find({i[0], i[1]}) == rem.end())
            dsu.unionSets(i[0], i[1]);
    vector<string> ans;
    reverse(all(queries));
    for (auto &i : queries)
        if (i[0] == 0)
            ans.push_back(dsu.find(i[1]) == dsu.find(i[2]) ? "YES\n" : "NO\n");
        else
            dsu.unionSets(i[1], i[2]);
    reverse(all(ans));
    for (auto i : ans)
        cout << i;
}
